...
public interface BusinessDefineRep extends CrudRepository<BusinessDefine,String> {
    ...
    @Query("SELECT distinct new cc.coopersoft.business.model.BusinessDefineSummary(b.id,b.name,c.name,b.priority,c.priority,b.memo) from BusinessDefine b left join b.createRoles r left join b.businessCategory c where r.role in ( :roles ) order by b.businessCategory.priority, b.priority")
    List<BusinessDefineSummary> listDefineSummaryByRole(@Param("roles") Collection<String> roles);

    @Query("SELECT distinct new cc.coopersoft.business.model.BusinessDefineSummary(b.id,b.name,c.name,b.priority,c.priority,b.memo) from BusinessDefine b left join b.createRoles r left join b.businessCategory c order by b.businessCategory.priority, b.priority")
